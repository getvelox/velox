...
- [x] **payment_scheduled**: `auto_charge_pending=true` → muted banner, action **Charge now**. *(re-walked 2026-07-28 with NO fixture poke, on VLX-000002 (Walkthrough Co): attached a real card to a card-less customer through a real Stripe Checkout setup session, which is the product's own recovery path — the four invoices already queued for that customer flipped from `no_payment_method` to this banner on their own. Muted banner, code `payment.scheduled`, single action **Charge now**, clock-aware copy "Auto-charge runs on the next test-clock advance", Due-by row. Verified the promise is TRUE rather than assumed: the catchup sweep's predicate has no dunning-run filter, so it re-charges these invoices even though their recovery campaign already escalated.)*
