...
- [x] **ADR-104 walk: live invoices are untouched** — a wall-clock invoice's timeline shows one lane, emails at their real times, **no** Recorded sublines, and no simulation vocabulary anywhere on the page. *(walked 2026-07-29 on VLX-000031, a wall-clock paid invoice: one Activity card — created → finalized → recovery started → "Customer notified — set up payment method" → "Payment recorded (offline) $4.15" — all at real Jul 29 times, zero `Recorded <time>` sublines, no banner. NOTE: this page also uses "recorded" in its OTHER product sense (manual out-of-band payment entry); ADR-104 Invariant C records the collision and why the timeline word still wins.)*
