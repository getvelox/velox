...
- [ ] On invoice detail page activity for a clock-pinned in_arrears sub with a known-failed charge at cycle close: lifecycle events (Invoice created → Invoice finalized) render BEFORE dunning events ("Payment failed — automatic retry scheduled").
...
- [x] **Failure rows fold inside-out**: each failed charge collapses to ONE row carrying the dunning attempt label ("Payment failed — automatic retry scheduled" or "Payment retry #N attempted"), the PI id, the amount, the decline reason, and a `Customer notified by email` sub-line. Card-less enrollments (no charge ever attempted) render "No payment method — reminder cycle started" instead — the start row carries its recorded cause. *(walked 2026-07-27: NIM-000253's 0341 decline folded into the dunning row with pi_3TxXaE…, payment_failed reason, and the email sub-line — no separate Stripe row.)* No separate Stripe `payment_intent.payment_failed` row at the same instant; no separate "Payment-failed email sent" row beneath.
