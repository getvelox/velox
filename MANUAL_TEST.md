...
- [ ] **ADR-104 walk: teardown takes the emails** — delete a test clock → its customers' `email_outbox` rows (anchored by `test_clock_id`) are gone with the rest of the graph; the customer page shows no orphan sent-emails from the deleted simulation. CI-locked: the teardown completeness test now discovers `test_clock_id`-carrying tables.
