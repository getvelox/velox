...
- [x] **Failure rows fold inside-out**: each failed charge collapses to ONE row carrying the dunning attempt label ("Automatic retry scheduled" or "Payment retry #N attempted"), the PI id, the amount, the decline reason, and a `Customer notified by email` sub-line. *(walked 2026-07-27: NIM-000253's 0341 decline folded into the dunning row with pi_3TxXaE…, payment_failed reason, and the email sub-line — no separate Stripe row.)* No separate Stripe `payment_intent.payment_failed` row at the same instant; no separate "Payment-failed email sent" row beneath.
