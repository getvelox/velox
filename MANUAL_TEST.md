...
> **Blocked in a single session (assessed 2026-07-29).** The three boxes below need a *retryable* tax failure — `provider_outage` / `provider_not_configured` — and this tenant has Stripe connected and healthy, so the only failure reachable by configuration is `customer_data_invalid`, which is deliberately NOT in `TaxRetryableErrorCodes()` and so never enters the scan at all. Producing a retryable one means disconnecting Stripe, and that is **not recoverable here**: the tenant's secret key (`sk_test_51TO…vnhJ`) differs from the one the local Stripe CLI holds, and the webhook signing secret is not retrievable, so a disconnect would silently break event ingestion for every other flow. Separately, the backoff schedule is wall-clock (`[5min, 15min, 1h, 4h, 12h, …]`) and test clocks do not drive the wall-clock reconciler — reaching 8 attempts is **hours** of real time, not a clock advance. These need a seeded fixture or a fault-injection hook, not a walk.
