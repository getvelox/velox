...
- [ ] **Dunning-off failure lands in Activity at simulated time (ADR-102)**: on a clock-pinned invoice whose auto-charge declined while dunning was disabled, the **Activity** card shows "Payment failed" with the provider reason at the **simulated** instant (the charge-attempt fact) — not silence — and the wall-clock lane does NOT show a duplicate "Payment failed" webhook row (the attempt replaced its echo, lifting the "Customer notified by email" sub-line when one folded). With dunning ON, rendering is unchanged (recovery row owns the failure). The second card is always titled "Real-time activity" — it never renames itself. *(walked 2026-07-28 on NIM-000269: Standard policy disabled + Mailpit stopped, 0341 card auto-charge declined at cycle close → zero dunning runs; timeline kept the standalone "Payment failed / Your card was declined. / $4.40" row AND "Payment-failed email sent to customer (queued)" with the SMTP connect error; policy re-enabled + Mailpit restarted → the queued email healed to dispatched on the outbox retry. Side catch: settlement logged "dunning started" on the deliberate-skip path — startDunningWithRetry now reports started=false and the log says skipped.)*
