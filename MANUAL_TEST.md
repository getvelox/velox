...
- [ ] Backoff respected: the 1st retry is ~5min ahead (±10% jitter), the 5th ~12h ahead (schedule `[5m, 15m, 1h, 4h, 12h, 24h, 48h, 96h]`). Sub-5-min ticks don't double-process the row. *(IN PROGRESS 2026-07-29, first two steps observed live on both scratch invoices: after retry #1 next=+5m01s; the 11:14:29 tick correctly SKIPPED rows due at 11:14:36/39 (no double-process); the 11:19 tick fired retry #2 with next=+14m20s/+15m05s — the 15m step inside ±10% jitter. TaxLadder Co left climbing for the 1h/4h/12h tail; full exhaustion ≈ Aug 6.)*
