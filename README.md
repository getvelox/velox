...
- **Bad debt, decided rather than inherited (Aug 2026, ADR-110-112)** — writing an invoice off no longer reverses its tax (that is a relief claim your business makes, on conditions we don't know); a written-off invoice can be charged when the customer comes back, as an operator action only; and a dunning policy now sets the subscription's fate and the invoice's fate **separately**, so "cancel and write off" is expressible and a policy that leaves the debt open says so on the form instead of leaving a receivable nothing ever closes
