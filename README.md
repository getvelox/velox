...
**What we have actually measured, and what we haven't.** On AWS (`c7g.2xlarge` app + `db.m7g.2xlarge` RDS, one AZ) the ingest API held **10,203 events/sec** over 90 s and **1,000 events/sec at p99 269 ms** through a 10-minute soak, both with 0 errors — but every one of those figures is a **single run**, and its event count came from the load generator rather than the database. The methodology and rig are in [`docs/benchmarks/sustained-throughput.md`](docs/benchmarks/sustained-throughput.md), and the load generator that produced those numbers has since been **replaced** (k6) after a pacing bug was found in it, so the doc says plainly which figures predate the fix. Re-measuring under the current protocol — discarded warmup, repeats with spread, every run reconciled against `usage_events` — is what replaces them. Treat the numbers as an honest first measurement with its weaknesses stated, not as a specification.
