...
- **Bad debt, decided rather than inherited (Aug 2026, ADR-110-113)** — writing an invoice off no longer reverses its tax (that is a relief claim your business makes, on conditions we don't know); a dunning policy sets the subscription's fate and the invoice's fate **separately**, so "cancel and write off" is expressible and a policy that leaves the debt open says so on the form; and nothing charges a written-off invoice — recovery of a returned customer runs on normal rails via a fresh recovery invoice, while offline payments and provider-confirmed charges still settle it
