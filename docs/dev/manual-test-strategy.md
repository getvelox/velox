...
**What this is:** the working discipline for "walking" a flow — manually
exercising a product flow end to end and recording evidence against its
checklist boxes in `MANUAL_TEST.md`. **Who it is for:** anyone who has to prove
a flow behaves, whether they have worked in this repo for a year or a day.
...
playbook governs **how to build** a money-path change (one touching invoices,
payments, credits, or another money-moving state machine); this doc governs
**how to prove** one behaves.

Every technique below is here because it caught a real defect in this codebase.
Where a technique names a PR (pull request), that PR is the bug it found.
Nothing in this doc is aspirational.
...
1. **A box is not walked until it could have failed.** A box is one checkbox
   assertion in `MANUAL_TEST.md`. If no outcome of the exercise would have made
   you write "this is broken", you performed a demonstration, not a test.
2. **Assert the consequence, not the call.** `200 OK` is not a passing money
   ...
   screen (what the human running the dashboard sees) are the assertions. This
   does **not** mean "ignore the code" — see §2. Observation decides whether
   behaviour is right; reading explains why and where the fix goes; and some
   defects are invisible to observation entirely.
...
(checks that only read the page's markup or text) are *structurally incapable*
of seeing layout: overflow, containment, truncation, z-index, obscured
controls. It cost two operator-reported defects before it was written down
(see PR #669: a dialog popover spilled ~110px past both edges while every text
assertion passed).

The **Design** lens earns its keep on flows that pass, too: a documented "this
is correct and here is why, and here are the 2–4 platforms that agree" is a
first-class outcome, not a consolation prize.
...
This clause exists because a question exposed the imbalance: the original
catalogue below leans observational, with code-reading appearing only as §3.6.
Every bug FLOW C (a named flow family in `MANUAL_TEST.md`) found was *observed*
— and every one of them was *diagnosed by reading*.

### Code is never evidence that behaviour is correct

The tax-reconciler bug (#661) is the cautionary case (a reconciler is a
background scan that retries work that previously failed):
`TaxRetryableErrorCodes()` **declared itself** the single source and listed
three codes; the operator banner classified from it and promised a retry; both
scan call sites hardcoded a shorter list. Reading either the declaration or
the banner yields a confident **false pass**. Only the observation settled it —
twelve consecutive ticks (scheduler runs) with `retry_count` still 0.
...
**block the downgrade** while a charge is in flight (initiated but not yet
settled) — wrong, because a bank-debit source can be in flight for days, so
that blocks a customer's plan change the whole time. The code said the correct
...
behaviour: the dunning badge (dunning: the automated follow-up on failed
payments), the tax scan, the clawback scan (clawback: reclaiming previously
granted credit) and the refund-attention filter all carry
`AND is_simulated = false` — simulated fixtures are excluded by design
(sim-gating, decided in ADR-086; an ADR is an architecture decision record).
Without reading the predicate, each looks like a bug. Before
...
Applies to: refunds, charges, Stripe Checkout sessions (hosted payment pages;
`open` vs `expired` — how the StopCollection bug was proven), tax transactions,
webhook delivery.
...
Some states are only reachable by force, and the box should say so. Forcing
them with a direct database write (`psql`, the Postgres command-line client) is
legitimate **when no product flow can produce them** — an
...
is empty, or when there is exactly one?" On a frozen test clock (a simulated
billing clock pinned to one instant), *every* row shares an instant — so
same-instant behavior isn't an edge case, it's the common case.
...
A backoff ladder (retry delays that grow with each attempt), a lease expiry, a
delivery verdict arriving days later: these cannot be asserted at t=0. Arm the
fixture, record what you expect and when, and come back. Say so in the box
rather than claiming the untested rungs.
...
A "blocked" note (a box annotated as currently un-walkable) is a claim with an
expiry date. Re-derive it before trusting it.
...
the moment the scheduler processes it." A test-clock advance cannot walk these
— the catchup (the scheduler's make-up pass for the skipped time) runs
synchronously inside the advance, closing exactly the window under test.
Shrinking the scheduler interval walks a *different configuration* than
...
3. **Exercise the real path** — dashboard for UI claims, API for contract
   claims. `psql` only per §3.8 (adversarial states no product flow can
   produce).
...
- **One purpose-built tenant per campaign.** A tenant is an isolated customer
  workspace; a campaign is one sustained walk of a flow family. Aggregate
  surfaces (stat cards, badges, lists) can only be *asserted* when the tenant's
  contents are fully known; in an aged tenant "the number renders" is the only
  checkable claim.
...
3. **Would the external contracts agree this state is possible?** Check the
   provider (a PI (PaymentIntent) id must exist at Stripe — I4d's own setup
   line said `'pi_x'` and could not have passed its boxes), database triggers
   (migration 0021's trigger silently overwrites `livemode` on bypass inserts),
   and RLS (row-level security; here, mode scoping) *before* the write — each
   of these produced a false alarm or a dead fixture when skipped.
...
- the fixture identity — invoice, credit note (CN), or subscription (sub)
  number, plus clock position
...
- the CI (continuous-integration) test name where a leg of the claim is
  automated rather than hand-walked
...
- **Automate** concurrency, money invariants, and any bug class that has now
  bitten twice. Three drifts of one class ⇒ mechanise a gate (turn the manual
  check into an automated one).
...
This doc supersedes the scattered guidance in these session-memory notes,
which now point here: manual-test currency · prefer-real-flow-over-DB ·
walkthrough design-review trigger (incl. the visual-QA clause) · verify-real-path ·
