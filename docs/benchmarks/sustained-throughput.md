...
Load is generated by k6, not by anything in this repo's Go tree:

```bash
velox-bench-seed                            # prints {"api_key": …} — fixtures + key
k6 run -e BASE=http://<app>:8080 -e API_KEY=… \
       -e RATE=1000 -e BATCH=10 -e DURATION=10m scripts/bench-rig/ingest.js
```

`RATE` is events/sec, not requests/sec — the script divides by `BATCH` itself,
because events/sec is the number that gets published. **Read `dropped` before
reading any percentile**: a non-zero value means the offered rate was never
actually offered, so the run is a failed attempt at that rate rather than a
measurement of it.
