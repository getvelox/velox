...
[Appendix C](#appendix-c--mechanisms). Series evidence:
[run 3](#third-run-2026-08-17-what-a-tail-event-is-caught-live).
...
## Appendix A — the run journal

The chapters below are the runs as they happened, kept intact — including the
readings that were later revised, because the revisions are part of the
result. Each chapter states its
per-run specifics; throughout, every repeat was gated and every event
reconciled as stated in [Method](#method-and-the-gates--stated-once), so a
chapter stands alone if linked directly.
...
*(The WAL mechanism, why the pool runs dry, and the round-trip cost model that this chapter originally carried inline are in [Appendix C](#appendix-c--mechanisms).)*
