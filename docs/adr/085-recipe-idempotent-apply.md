...
2. **Adopt-or-create the shared catalog.** Rating rules adopt an existing `rule_key` at its current version, never republish (see Money safety #2) — and only if its money fields match the recipe's declaration, else refuse loud (2026-07-26 amendment under "Never republish"). Meters adopt an existing `key` only if `aggregation` matches, else refuse loud (see Money safety #3).
...
**Amendment (2026-07-26): rule adoption is conformance-gated, symmetric with the meter guard.** As originally shipped, adoption was deliberately ungated — any same-key rule was bound at its current version regardless of its rates. That protected live subs (no republish) but let the *preview lie*: the dialog showed the recipe's declared rates while the installed plan billed the existing rule's. Same silent-substitution class this ADR killed for plans, so the same remedy: `ruleConformanceDiff` compares the money fields (mode, currency, per-unit rate, tiers, package size/amount, overage rate) against the key's current active version, and a divergence refuses loud — `errs.AlreadyExists("rating_rule", …)` naming each diverging field, whole tx rolled back. An exactly-matching rule still adopts without republishing (CI-locked both directions: `TestService_Instantiate_RefusesDivergentRatingRule` / `AdoptsMatchingRatingRule`). Apply remains additive, net-new only.
